...
### 数据集成首页修改
*需求*
1. 数据集成首页提供三种创建同步任务的基础选项，不需要显示同步范围，用户可以在任务详情页配置同步范围
2. 数据任务详情页，替换数据源选项为数据源管理中已创建的数据源
3. 数据任务详情页，替换数据库选项为所选数据源下已检测到的数据库，如果没有数据库，则不显示该选项
...
*其他规范*
- 前端更新代码位置： frontend\src\views\dataintegration
- 查询数据源，数据库，数据表的接口均在database.js中，分别为listDatasource,listBusinessDatabases，listBusinessTables
- 不修改后端代码位置
