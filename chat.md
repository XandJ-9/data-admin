...
# 数据集成功能开发
*需求*
  数据集成同步功能模块，包括单表离线同步和分库分表离线同步
  - 单表离线同步：支持将单个数据库中的表数据同步到另一个数据库中的表
  - 分库分表离线同步：支持将多个数据库中的表数据同步到另一个数据源的的单个表中
...
- 每种类型提供一个创建任务按钮，点击按钮跳转到任务详情页进行编辑,任务详情页会根据选择的类型加载对应的同步详情组件

- 单表离线同步详情
  - 选择数据源，数据源，数据库（如果存在），数据表
  - 选择目标数据源，数据库（如果存在），数据表
  - 目标数据源不能与来源数据源一致

- 分库分表离线同步详情
  - 选择数据源，数据源，数据库(支持多选和自定义名称正则模式匹配)，数据表(支持多选和自定义名称正则模式匹配)
  - 选择目标数据源，数据库（如果存在），数据表（如果不存在，支持自定义表名）
  - 目标数据数据源不能与来源数据源一致

- 同步任务公共配置
  - 配置字段映射关系，例如将源表的字段A映射到目标表的字段B
  - 支持默认字段映射，例如将源表所有字段映射到目标表同名字段
  - 在目标表的配置中允许新增字段，同时对应的源表字段可以自定义
  - 允许编写where条件
  - 调度策略（定时或手动）等信息
  - 数据同步方式（增量同步或全量同步），当选择增量同步时，可以配置增量字段（自增ID或时间戳字段），支持自定义增量字段


*输出代码要求*
- 数据集成首页：frontend/src/views/dataintegration/index.vue
- 任务详情页taskDetail.vue
- 前端将单表离线同步详情，分库分表离线详情分别封装成组件，置于目录：frontend/src/views/dataintegration/offline/components
