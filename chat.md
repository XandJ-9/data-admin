...
## 元数据管理
*目标：管理数据源的元数据，包括库、表、字段等信息*

后端django新增应用datameta，用于管理数据源的元数据，元数据管理页面展示已纳管数据源的元数据，元数据管理页面手动触发元数据采集。
代码位置：
- 后端：backend/apps/datameta
- 前端：frontend/src/views/datameta

*需求*
- 用户可以在元数据管理页面查看已纳管的数据源的元数据，包括库、表、字段等信息。
- 用户可以在元数据管理页面手动触发元数据采集，采集已纳管数据源的元数据信息。
