...
- 总体结构
  - `backend/hrms/`：项目配置（`settings.py`、`urls.py`、`wsgi.py`、`asgi.py`）
...
- 统一响应格式（成功）
  ...
  - 路由：
    ...
    ```
- 统一错误包装
  - 全局异常处理：在 `hrms.settings` 的 `REST_FRAMEWORK` 中设置 `EXCEPTION_HANDLER = 'system.exceptions.custom_exception_handler'`
  - 行为：
...
- 认证方式
  - JWT：`DEFAULT_AUTHENTICATION_CLASSES = ('rest_framework_simplejwt.authentication.JWTAuthentication',)`（`hrms.settings`）
- 鉴权
...
- 统一异常入口：`system.exceptions.custom_exception_handler`（在 `hrms.settings.REST_FRAMEWORK` 中配置）
...
- 全局配置与异常：`hrms.settings.REST_FRAMEWORK`、`system.exceptions.custom_exception_handler`
...
- 集成 `drf-spectacular` 并在 `hrms.urls` 暴露 `/api/schema` 与 `/api/docs`
