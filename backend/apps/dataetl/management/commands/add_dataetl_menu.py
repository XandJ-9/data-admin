...
运行方式:
    python manage.py add_dataetl_menu
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('\n' + '='*60)
        if force_update:
            self.stdout.write('[更新] 开始更新数据ETL管理菜单（强制模式）')
        else:
            self.stdout.write('[添加] 开始添加数据ETL管理菜单')
        ...
        try:
            with transaction.atomic():
                ...
                # 2. 创建/更新 ETL任务管理菜单
                self.stdout.write('[步骤 2/4] 处理 ETL任务管理 菜单...')
                self.create_task_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   [成功] 处理完成\n'))

                # 3. 创建/更新 字段映射管理菜单
                self.stdout.write('[步骤 3/4] 处理 字段映射管理 菜单...')
                self.create_field_mapping_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   [成功] 处理完成\n'))

                # 4. 创建/更新 执行日志菜单
                self.stdout.write('[步骤 4/4] 处理 执行日志 菜单...')
                self.create_execution_log_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   [成功] 处理完成\n'))

                self.stdout.write('='*60)
                if force_update:
                    self.stdout.write(self.style.SUCCESS('[完成] 数据ETL管理菜单更新完成！'))
                else:
                    self.stdout.write(self.style.SUCCESS('[完成] 数据ETL管理菜单添加完成！'))
                self.stdout.write('='*60)
                self.stdout.write('\n菜单结构：')
                self.stdout.write('  数据ETL管理')
                self.stdout.write('  |-- ETL任务管理')
                self.stdout.write('  |-- 字段映射管理')
    ...
    def create_parent_menu(self, force_update=False):
        ...
        if force_update:
            # 强制更新模式：先查找再更新
            try:
                menu = Menu.objects.get(menu_name='数据ETL管理')
                menu.parent_id = 0
                menu.order_num = 7
                menu.path = '/data-etl'
                menu.component = ''
    ...
    def create_task_menu(self, parent_id, force_update=False):
        """创建或更新 ETL任务管理 菜单"""
    ...
    def create_field_mapping_menu(self, parent_id, force_update=False):
        """创建或更新 字段映射管理 菜单"""
        self.create_or_update_menu(
            menu_name='字段映射管理',
            parent_id=parent_id,
    ...
    def create_execution_log_menu(self, parent_id, force_update=False):
        """创建或更新 执行日志 菜单"""
        self.create_or_update_menu(
            menu_name='执行日志',
            parent_id=parent_id,
