from django.core.management.base import BaseCommand
from django.utils import timezone

from apps.system.models import Menu, Role, User, UserRole, RoleMenu


class Command(BaseCommand):
    help = "Initialize menus, roles, users and bindings: 菜单、角色、用户、一键初始化"

    def add_arguments(self, parser):
        parser.add_argument(
            '--force',
            action='store_true',
            help='强制更新已存在的菜单、角色、用户等数据'
        )
        parser.add_argument(
            '--skip-users',
            action='store_true',
            help='跳过用户初始化（仅初始化菜单和角色）'
        )

    def handle(self, *args, **options):
        force_update = options.get('force', False)
        skip_users = options.get('skip_users', False)
        now = timezone.now()

        if force_update:
            self.stdout.write(self.style.WARNING('强制更新模式：将更新所有已存在的数据'))
        if skip_users:
            self.stdout.write(self.style.WARNING('跳过用户初始化'))

        # 初始化系统管理目录
        root_defaults = {
            'menu_name': '系统管理',
            'order_num': 1,
            'component': '',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'M',
            'visible': '0',
            'status': '0',
            'perms': '',
            'icon': 'system',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '系统管理目录',
            'del_flag': '0'
        }
        # get_or_create: 如果记录存在则获取，不存在则使用defaults插入新数据
        # created_root=True: 新插入的记录
        # created_root=False: 已存在的记录
        root, created_root = Menu.objects.get_or_create(parent_id=0, path='/system', menu_type='M', defaults=root_defaults)

        # 只有当记录已存在且需要强制更新时，才更新数据
        if not created_root and force_update:
            for k, v in root_defaults.items():
                setattr(root, k, v)  # 强制更新使用新值
            root.save()
        
        # 初始化用户管理菜单
        user_defaults = {
            'menu_name': '用户管理',
            'order_num': 1,
            'component': 'system/user/index',
            'route_name': 'User',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'C',
            'visible': '0',
            'status': '0',
            'perms': 'system:user:list',
            'icon': 'user',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '用户管理菜单',
            'del_flag': '0'
        }
        user_menu, created_user_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='user', menu_type='C', defaults=user_defaults)
        if not created_user_menu and force_update:
            for k, v in user_defaults.items():
                setattr(user_menu, k, v)  # 强制更新使用新值
            user_menu.save()

        # 初始化角色管理菜单
        role_defaults = {
            'menu_name': '角色管理',
            'order_num': 2,
            'component': 'system/role/index',
            'route_name': 'Role',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'C',
            'visible': '0',
            'status': '0',
            'perms': 'system:role:list',
            'icon': 'peoples',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '角色管理菜单',
            'del_flag': '0'
        }
        role_menu, created_role_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='role', menu_type='C', defaults=role_defaults)
        if not created_role_menu and force_update:
            for k, v in role_defaults.items():
                setattr(role_menu, k, v)  # 强制更新使用新值
            role_menu.save()

        # 初始化部门管理菜单
        dept_defaults = {
            'menu_name': '部门管理',
            'order_num': 3,
            'component': 'system/dept/index',
            'route_name': 'Dept',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'C',
            'visible': '0',
            'status': '0',
            'perms': 'system:dept:list',
            'icon': 'tree-table',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '部门管理菜单',
            'del_flag': '0'
        }
        dept_menu, created_dept_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='dept', menu_type='C', defaults=dept_defaults)
        if not created_dept_menu and force_update:
            for k, v in dept_defaults.items():
                setattr(dept_menu, k, v)  # 强制更新使用新值
            dept_menu.save()

        # 初始化菜单管理菜单
        menu_defaults = {
            'menu_name': '菜单管理',
            'order_num': 4,
            'component': 'system/menu/index',
            'route_name': 'Menu',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'C',
            'visible': '0',
            'status': '0',
            'perms': 'system:menu:list',
            'icon': 'tree',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '菜单管理菜单',
            'del_flag': '0'
        }
        menu_menu, created_menu_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='menu', menu_type='C', defaults=menu_defaults)
        if not created_menu_menu and force_update:
            for k, v in menu_defaults.items():
                setattr(menu_menu, k, v)  # 强制更新使用新值
            menu_menu.save()

        # 初始化字典管理菜单
        dict_defaults = {
            'menu_name': '字典管理',
            'order_num': 5,
            'component': 'system/dict/index',
            'route_name': 'Dict',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'C',
            'visible': '0',
            'status': '0',
            'perms': 'system:dict:list',
            'icon': 'dict',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '字典管理菜单',
            'del_flag': '0'
        }
        dict_menu, created_dict_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='dict', menu_type='C', defaults=dict_defaults)
        if not created_dict_menu and force_update:
            for k, v in dict_defaults.items():
                setattr(dict_menu, k, v)  # 强制更新使用新值
            dict_menu.save()

        # 初始化系统配置管理菜单
        config_defaults = {
            'menu_name': '系统配置管理',
            'order_num': 6,
            'component': 'system/config/index',
            'route_name': 'Config',
            'query': '',
            'is_frame': '1',
            'is_cache': '0',
            'menu_type': 'C',
            'visible': '0',
            'status': '0',
            'perms': 'system:config:list',
            'icon': 'edit',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'remark': '系统配置管理菜单',
            'del_flag': '0'
        }
        config_menu, created_config_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='config', menu_type='C', defaults=config_defaults)
        if not created_config_menu and force_update:
            for k, v in config_defaults.items():
                setattr(config_menu, k, v)  # 强制更新使用新值
            config_menu.save()

        # 初始化角色
        role_admin_defaults = {
            'role_name': '管理员',
            'role_key': 'admin',
            'role_sort': 1,
            'data_scope': '1',
            'menu_check_strictly': 1,
            'dept_check_strictly': 1,
            'status': '0',
            'remark': '',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'del_flag': '0',
        }
        role_admin, created_role_admin = Role.objects.get_or_create(role_key='admin', defaults=role_admin_defaults)
        if not created_role_admin and force_update:
            for k, v in role_admin_defaults.items():
                setattr(role_admin, k, v)  # 强制更新使用新值
            role_admin.save()

        role_common_defaults = {
            'role_name': '普通用户',
            'role_key': 'common',
            'role_sort': 2,
            'data_scope': '1',
            'menu_check_strictly': 1,
            'dept_check_strictly': 1,
            'status': '0',
            'remark': '',
            'create_by': 'system',
            'update_by': 'system',
            'create_time': now,
            'update_time': now,
            'del_flag': '0',
        }
        role_common, created_role_common = Role.objects.get_or_create(role_key='common', defaults=role_common_defaults)
        if not created_role_common and force_update:
            for k, v in role_common_defaults.items():
                setattr(role_common, k, v)  # 强制更新使用新值
            role_common.save()

        msg = []
        msg.append('管理员角色已存在' if not created_role_admin else '管理员角色已创建')
        msg.append('普通用户角色已存在' if not created_role_common else '普通用户角色已创建')
        self.stdout.write(self.style.SUCCESS('；'.join(msg)))

        # 初始化用户
        if not skip_users:
            admin_defaults = {
                'nick_name': '管理员',
                'email': 'admin@example.com',
                'is_staff': True,
                'is_superuser': True,
                'status': '0',
                'create_by': 'system',
                'update_by': 'system',
                'create_time': now,
                'update_time': now,
                'del_flag': '0',
            }
            admin_user, created_admin_user = User.objects.get_or_create(username='admin', defaults=admin_defaults)
            if created_admin_user:
                admin_user.set_password('admin123')
                admin_user.save()
            elif force_update:
                for k, v in admin_defaults.items():
                    setattr(admin_user, k, getattr(admin_user, k) if getattr(admin_user, k) is not None else v)
                admin_user.is_staff = True
                admin_user.is_superuser = True
                admin_user.status = '0'
                admin_user.del_flag = '0'
                admin_user.update_time = now
                admin_user.save()

            test_defaults = {
                'nick_name': '测试用户',
                'email': 'test@example.com',
                'is_staff': False,
                'is_superuser': False,
                'status': '0',
                'create_by': 'system',
                'update_by': 'system',
                'create_time': now,
                'update_time': now,
                'del_flag': '0',
            }
            test_user, created_test_user = User.objects.get_or_create(username='test', defaults=test_defaults)
            if created_test_user:
                test_user.set_password('test123')
                test_user.save()
            elif force_update:
                for k, v in test_defaults.items():
                    setattr(test_user, k, getattr(test_user, k) if getattr(test_user, k) is not None else v)
                test_user.is_staff = False
                test_user.is_superuser = False
                test_user.status = '0'
                test_user.del_flag = '0'
                test_user.update_time = now
                test_user.save()

            msg = []
            msg.append('admin 已存在' if not created_admin_user else 'admin 已创建 密码: admin123')
            msg.append('test 已存在' if not created_test_user else 'test 已创建 密码: test123')
            self.stdout.write(self.style.SUCCESS('；'.join(msg)))
        else:
            # 获取已存在的用户用于角色绑定
            admin_user = User.objects.filter(username='admin').first()
            test_user = User.objects.filter(username='test').first()
            if admin_user and test_user:
                self.stdout.write(self.style.SUCCESS('跳过用户初始化，使用已存在的用户'))
            else:
                self.stdout.write(self.style.WARNING('警告：未找到admin或test用户，角色绑定可能失败'))

        # 初始化用户角色绑定
        if admin_user and test_user:
            UserRole.objects.get_or_create(user=admin_user, role=role_admin, defaults={
                'create_by': 'system',
                'update_by': 'system',
                'create_time': now,
                'update_time': now,
                'del_flag': '0',
            })
            UserRole.objects.get_or_create(user=test_user, role=role_common, defaults={
                'create_by': 'system',
                'update_by': 'system',
                'create_time': now,
                'update_time': now,
                'del_flag': '0',
            })
        else:
            self.stdout.write(self.style.WARNING('跳过用户角色绑定（用户不存在）'))

        # 初始化角色菜单绑定（管理员角色绑定所有菜单）
        all_menus = Menu.objects.filter(del_flag='0')
        menu_count = 0
        for menu in all_menus:
            RoleMenu.objects.get_or_create(role=role_admin, menu=menu, defaults={
                'create_by': 'system',
                'update_by': 'system',
                'create_time': now,
                'update_time': now,
                'del_flag': '0',
            })
            menu_count += 1

        parts = []
        parts.append('菜单已初始化')
        parts.append('角色已初始化')
        if not skip_users:
            parts.append('用户已初始化')
            parts.append('用户角色绑定完成')
        parts.append(f'管理员角色菜单绑定完成（{menu_count}个菜单）')
        self.stdout.write(self.style.SUCCESS('；'.join(parts)))
