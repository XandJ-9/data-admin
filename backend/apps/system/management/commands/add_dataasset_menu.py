...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('\n' + '='*60)
        if force_update:
            self.stdout.write('🔄 开始更新数据资产管理菜单（强制模式）')
        else:
            self.stdout.write('📦 开始添加数据资产管理菜单')
        self.stdout.write('='*60 + '\n')

        try:
            with transaction.atomic():
                # 1. 创建/更新一级菜单
                self.stdout.write('📝 步骤 1/5: 处理一级菜单...')
                ...
                # 2. 创建/更新数据资产概览菜单
                self.stdout.write('📝 步骤 2/5: 处理数据资产概览菜单...')
                self.create_overview_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   ✓ 处理成功！\n'))

                # 3. 创建/更新数据源管理菜单
                self.stdout.write('📝 步骤 3/5: 处理数据源管理菜单...')
                self.create_datasource_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   ✓ 处理成功！\n'))

                # 4. 创建/更新元数据浏览菜单
                self.stdout.write('📝 步骤 4/5: 处理元数据浏览菜单...')
                self.create_metadata_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   ✓ 处理成功！\n'))

                # 5. 创建/更新表血缘管理菜单
                self.stdout.write('📝 步骤 5/5: 处理表血缘管理菜单...')
                self.create_lineage_menu(parent_menu.menu_id, force_update)
                self.stdout.write(self.style.SUCCESS('   ✓ 处理成功！\n'))

                self.stdout.write('='*60)
                if force_update:
                    self.stdout.write(self.style.SUCCESS('✅ 数据资产管理菜单更新完成！'))
                else:
                    self.stdout.write(self.style.SUCCESS('✅ 数据资产管理菜单添加完成！'))
                self.stdout.write('='*60)
                self.stdout.write('\n菜单结构：')
                self.stdout.write('  数据资产管理')
                self.stdout.write('  ├── 数据资产概览')
                self.stdout.write('  ├── 数据源管理')
                self.stdout.write('  ├── 元数据浏览')
                self.stdout.write('  └── 表血缘管理（新功能）')
                self.stdout.write('\n请刷新页面查看菜单！\n')
    ...
    def create_parent_menu(self, force_update=False):
        ...
        if force_update:
            ...
            try:
                ...
                menu.save()
                menu.action = 'updated'
                self.stdout.write(f'   ℹ️  已存在菜单，执行更新\n')
    ...
    def create_overview_menu(self, parent_id, force_update=False):
        """创建或更新数据资产概览菜单"""
        self.create_or_update_menu(
            ...
            order_num=1,
            path='asset',
            component='data/asset/index',
            route_name='DataAssetIndex',
            icon='dashboard',
    ...
    def create_or_update_menu(self, menu_name, parent_id, order_num, path, component,
        ...
        if force_update:
            ...
            try:
                ...
                menu.save()
                self.stdout.write(f'   🔄 更新菜单: {menu_name}\n')
            except Menu.DoesNotExist:
                # 不存在则创建
                Menu.objects.create(**defaults)
                self.stdout.write(f'   🆕 新建菜单: {menu_name}\n')
        else:
            ...
            if action == '已存在':
                self.stdout.write(f'   ✓ 菜单已存在: {menu_name}\n')
            else:
                self.stdout.write(f'   ✓ 新建菜单: {menu_name}\n')
    ...
    def show_statistics(self, parent_menu_id):
        ...
        try:
            ...
            for menu in children.order_by('order_num'):
                status_icon = '✅' if menu.status == '0' else '❌'
                visible_icon = '👁️' if menu.visible == '0' else '🚫'
                self.stdout.write(
