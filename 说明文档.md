...
- 数据源列表：`GET /datasource/list`
- 测试连接：`POST /datasource/test`
- 执行查询：`POST /datasource/query`
- 元数据表列表：`GET /datameta/table/list`
- 查看表字段（业务数据）：`POST /datameta/business/columns`
- 采集元数据：`POST /datameta/business/collect`

> 具体路由以后端 `backend/config/urls.py` 与各 `apps/*/urls.py` 为准，前端请求封装位于 `frontend/src/api/*`。
