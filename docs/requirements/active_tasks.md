...
15. `dataintegration` 当前已拆分为“模块首页 + 任务列表”两个正式菜单入口：模块首页只承载任务规模、焦点任务与导航入口，筛选、执行、详情抽屉与执行记录统一收敛到“任务列表”页面，不再把总览和工作台堆在同一页面；首页视觉样式也已收敛到和 `dataasset`、`dataservice` 一致的浅色模块首页风格。
16. 系统菜单管理页当前已修正“修改菜单”表单回填逻辑：菜单详情会先与默认表单合并并做字段归一化，菜单树加载失败也会被新增/修改入口正确兜底，避免显示状态、菜单状态等字段展示不全或产生未处理异常。
