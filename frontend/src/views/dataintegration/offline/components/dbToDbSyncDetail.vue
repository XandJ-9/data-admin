  ...
  <div>
    <el-card>
      ...
      <el-row :gutter="20">
            ...
            <el-form :inline="true" :model="form.source" label-width="100px" label-position="left">
              <el-row>
                <el-form-item label="数据源">
                  <el-select v-model="form.source.dataSourceIds" style="width: 100%" multiple
                             filterable clearable collapse-tags placeholder="选择数据源">
                    <el-option v-for="ds in dsList" :key="ds.dataSourceId"
                               :label="ds.dataSourceName + ' (' + ds.dbType + ')'"
                               :value="ds.dataSourceId" />
                  </el-select>
                </el-form-item>
                <div v-if="sourceTypeError" style="color: #f56c6c; margin: 4px 0 8px 16px">来源数据源类型不一致（{{ sourceTypeText }}），不允许进行同步</div>
              </el-row>
              <el-row>
                <el-form-item v-if="dbList.length" label="数据库">
                  <el-select v-model="form.source.databases" value-key="key" style="width: 100%"
                             multiple filterable clearable collapse-tags placeholder="选择数据库">
              ...
              <el-row>
                <el-col>
                  <el-form-item label="数据表">
                    <el-select v-model="form.source.tables" value-key="key" style="width: 100%"
                               multiple filterable clearable collapse-tags placeholder="选择表">
            ...
            <el-form :inline="true" :model="form.target" label-width="100px">
              ...
              <el-row>
                <el-form-item v-if="targetDbList.length" label="数据库">
                  <el-select v-model="form.target.databaseName" style="width: 200px" filterable
                             clearable placeholder="选择数据库">
              ...
              <el-row>
                <el-form-item label="数据表">
                  <el-select v-model="form.target.tableName" style="width: 200px" filterable
                             allow-create default-first-option clearable placeholder="选择或输入表名">
        ...
        <div v-if="form.target.dataSourceId && form.source.dataSourceIds.length && form.source.dataSourceIds.includes(form.target.dataSourceId)"
...
function setForm(payload) {
  try {
    ...
    // 兼容单表旧结构
    if (data.source && data.source.dataSourceId) {
      form.source.dataSourceIds = data.source.dataSourceId ? [data.source.dataSourceId] : []
      pending.sourceDatabaseName = data.source.databaseName || ''
      pending.sourceTableName = data.source.tableName || ''
    } else {
      const src = data.source || {}
      form.source.dataSourceIds = Array.isArray(src.dataSourceIds) ? src.dataSourceIds : []
      // 若旧存储为字符串数组，延后匹配
      const dbs = Array.isArray(src.databases) ? src.databases : []
      form.source.databases = dbs.filter(d => typeof d === 'object')
      const tb = Array.isArray(src.tables) ? src.tables : []
      pending.sourceTableKeys = tb.filter(t => typeof t === 'string')
...
function applyDbPattern() {
  ...
  try {
    ...
    form.source.databases = displayDbList.value
...
watch(() => form.source.dataSourceIds.slice().join(','), () => {
  ...
  Promise.all(tasks).then(resList => {
    ...
    // 如果只有一个数据源且有旧数据待选数据库，尝试匹配设置
    if (ids.length === 1 && pending.sourceDatabaseName) {
      const match = aggregates.find(d => d.databaseName === pending.sourceDatabaseName)
      if (match) form.source.databases = [match]
    }

    // 判断是否有数据库，没有数据库则直接查询数据源下的表
    if (!hasDb) {
      ...
      Promise.all(tableTasks).then(results => {
        ...
        if (pending.sourceTableName) {
          const m = tables.find(t => t.tableName === pending.sourceTableName)
          if (m) form.source.tables = [m]
        }
      })
    }
  })
...
watch(() => form.source.databases.map(d => d.key).join(','), () => {
  ...
  listTables({ dataSourceId: firstDb.dataSourceId, databaseName: firstDb.databaseName }).then(res => {
    const tables = []
    ;(res.rows || []).forEach(r => {
      const name = r.tableName || r
      const key = String(firstDb.dataSourceId) + ':' + String(firstDb.databaseName) + ':' + String(name)
      tables.push({ key, dataSourceId: firstDb.dataSourceId, dataSourceName: firstDb.dataSourceName, databaseName: firstDb.databaseName, tableName: String(name), label: firstDb.dataSourceName + ' / ' + firstDb.databaseName + ' / ' + String(name) })
    })
    tableList.value = tables
    displayTableList.value = tables
    if (pending.sourceTableName) {
      const m = tables.find(t => t.tableName === pending.sourceTableName)
      if (m) form.source.tables = [m]
    } else if (pending.sourceTableKeys && pending.sourceTableKeys.length) {
      form.source.tables = tables.filter(t => pending.sourceTableKeys.includes(t.key))
    }
  })
})
