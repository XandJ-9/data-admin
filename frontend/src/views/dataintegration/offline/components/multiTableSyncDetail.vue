                    ...
                    <el-card>
                        ...
                        <el-form :inline="true" :model="form.source" label-width="100px" label-position="left">
                            <el-row>
                                ...
                                <div v-if="sourceTypeError" style="color: #f56c6c; margin: 4px 0 8px 16px">来源数据源类型不一致（{{
                                    sourceTypeText
                                    }}），不允许进行同步</div>
                            </el-row>
                            <el-row>
                                <el-form-item v-if="dbList.length" label="数据库">
                                    <el-select v-model="form.source.databases" value-key="key" style="width: 100%"
                                        ...
                                        <el-option v-for="db in displayDbList" :key="db.key" :label="db.label"
                                            :value="db" />
                                    </el-select>
                                </el-form-item>
                                <el-form-item v-if="dbList.length" label="库名正则">
                                    <el-input v-model="form.source.databasePattern" style="width: 100%"
                                        placeholder="示例：^tenant_" />
                                    <el-button size="small" style="margin-left: 8px"
                                        @click="applyDbPattern">匹配</el-button>
                                </el-form-item>
                            </el-row>
                            <el-row>
                                <el-col>
                                    <el-form-item label="数据表">
                                        <el-select v-model="form.source.tables" value-key="key" style="width: 100%"
                                            ...
                                            <el-option v-for="t in displayTableList" :key="t.key" :label="t.label"
                                                :value="t" />
                                        </el-select>
                                    </el-form-item>
                                </el-col>
                                <el-col>
                                    <el-form-item label="表名正则">
                                        <el-input v-model="form.source.tablePattern" style="width: 100%"
                                            placeholder="示例：^order_|_log$" />
                                        <el-button size="small" style="margin-left: 8px"
                                            @click="applyTablePattern">匹配</el-button>
                                    </el-form-item>
                                </el-col>
                            </el-row>
                        </el-form>
                ...
                <el-col :span="12">
                    <el-card>
                        ...
                        <el-form :inline="true" :model="form.target" label-width="100px">
                            <el-row>
                                <el-form-item label="数据源">
                                    <el-select v-model="form.target.dataSourceId" placeholder="选择数据源"
                                        style="width: 200px">
                                        <el-option v-for="ds in dsList" :key="ds.dataSourceId"
                                            :label="ds.dataSourceName + ' (' + ds.dbType + ')'"
                                            :value="ds.dataSourceId" />
...
watch(() => form.source.databases.map(d => d.key).join(','), () => {
    ...
    // let selectedDbs = []
    // // 如果使用数据库名模式匹配，则只查询匹配中的第一个数据库表信息
    // if (!form.source.databasePattern) {
    //     selectedDbs = Array.isArray(form.source.databases) ? form.source.databases : []
    // } else {
    //     selectedDbs = form.source.databases[0] ? [form.source.databases[0]] : []
    // }
